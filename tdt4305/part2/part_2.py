...
def load(sql_context, input_dir, resources_dir):
    rt_df = loader.load_reviews(sql_context, input_dir, RT_FILENAME)
    afinn, stopwords = loader.load_sentiment_mapping(resources_dir)
    return rt_df, afinn, stopwords

def run(sql_context, input_dir, resources_dir, k=10):
    # Load dataset and sentiment mapping
    rt_df, afinn, stopwords = load(sql_context, input_dir, resources_dir)

    # Perform sentiment analysis and extract top 10 entries
    top_k_businesses = analysis.analyze(rt_df, afinn, stopwords, k)
...
def export_csv(spark_context, sql_context, input_dir, output_dir, resources_dir, extension, k=10):
    # Load prerequisites and run analysis
    rt_df, afinn, stopwords = load(sql_context, input_dir, resources_dir)
    results = analysis.analyze(rt_df, afinn, stopwords, k)
...
def export_txt(sql_context, input_dir, output_dir, resources_dir, k=10):
    ...
    with open(path, 'w') as sys.stdout:
        run(sql_context, input_dir, resources_dir, k)
...
def main(action, sample):
    ...
    if action == 'run':
        run(sql_context, input_dir, resources_dir)
    elif action == 'export-csv':
        export_csv(spark_context, sql_context, input_dir, output_dir, resources_dir, 'csv')
    elif action == 'export-tsv':
        export_csv(spark_context, sql_context, input_dir, output_dir, resources_dir, 'tsv')
    elif action == 'export-txt':
        export_txt(sql_context, input_dir, output_dir, resources_dir)
